import csv, os
base_onedrive = r'C:\Users\tbank\OneDrive\NOLO Video Scheduling\Current Project\Brand2Social Upload Chunks\7PM_2026-06-02_YouTube'
base_allowed = r'C:\Users\tbank\Desktop\Live Cowork\b2s_youtube_upload_chunks'
...
for i,u in enumerate(urls,1):
    ...
    title=f'The Open Door Encouragement {i}'
    rows=[rowset({'Schedule No':str(i),'Schedule Date/Time(DD/MM/YYYY HH:mm:ss)':date,'Common Description':desc,'Common First Comment':first,'Is Draft':'FALSE','Need Approval':'FALSE','Connected Profile Id':'11597','Connected Profile Name':'Eden Banks','Social Media':'youtube','Description':desc,'First Comment':first,'Post Type':'Shorts','Youtube Privacy':'Public','Youtube Title':title,'Media Url 1':url})]
    for base in (base_onedrive, base_allowed):
        out=os.path.join(base, f'B2S_7PM_youtube_chunk_{i:02d}.csv')
        with open(out,'w',newline='',encoding='utf-8') as f:
            w=csv.writer(f); w.writerow(header); w.writerows(rows)
    paths.append(os.path.join(base_onedrive, f'B2S_7PM_youtube_chunk_{i:02d}.csv'))
print('\n'.join(paths))
